...
const Navbar = ({ refs }) => {
  ...
  return (
    <nav
  className={`fixed top-0 left-0 w-full z-50 flex justify-between items-center px-4 py-2 text-white ${ranchers.className}`}
  style={{
    backgroundColor: '#020a08b5',
    paddingRight: 'calc(1rem + 8px)', // Adjust for scrollbar
  }}
>
  ...
  <ul
    ...
    } flex-col items-start absolute top-full left-0 w-full bg-[#4f0505] md:flex md:flex-row md:relative md:w-auto md:bg-transparent gap-4 sm:gap-6 md:gap-8 text-[16px] sm:text-[18px] tracking-wider font-semibold ${ranchers.className}`}
    style={{ paddingRight: '16px' }} // Adjust navbar list padding
  >
    ...
    <li className="flex items-center justify-center">
      <Link
        href="/webteam"
        className="hover:text-gray-400 transition-colors duration-300 py-2 px-6 md:px-0"
      >
        Web Team
      </Link>
    </li>
